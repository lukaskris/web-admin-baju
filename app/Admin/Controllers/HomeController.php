...
class HomeController extends Controller
{
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->description('19-08-2017');

            $content->row(function ($row) {
                //TODO: GET NEW USER IN THIS DAY
                // $category = Customer::select('id','name')->get();
                $row->column(3, new InfoBox('New Customers', 'users', 'aqua', '/admin/customer', '1024'));

                $row->column(3, new InfoBox('New Orders', 'shopping-cart', 'green', '/admin/orders', '150'));
                $row->column(3, new InfoBox('Pending Orders', 'book', 'yellow', '/admin/articles', '20'));
                $row->column(3, new InfoBox('Fail Orders', 'file', 'red', '/admin/files', '1'));
            });
